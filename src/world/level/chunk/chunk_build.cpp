...
void chunkBuildSection(ChunkMesh* c, const World* w, int si) {
    ...
    if (fast) {
        ...
        if (rc == 0) {
            ...
            s->mesh   = n0 ? chunkPackFinishSorted(g_stage[0], n0, n0, s->faceEnd) : 0; s->vertexCount = s->mesh ? n0 : 0;
            ...
            s->noMip  = n3 ? chunkPackFinishSorted(g_stage[3], n3, nLava, s->nmFaceEnd) : 0;
